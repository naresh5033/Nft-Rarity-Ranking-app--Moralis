...
api_key = "kXcuAnvE9jg9frBgOoYqQIJBHlLYVEoz5uCmh60brbWkvHAFFr3O7ePhm456g4Sk"

nftContract = "0x3110ef5f612208724ca51f5761a69081809f03b7" ##impostors genesis aliens
cursor = "" # the cursor will allow us to make multiple req to moralis api, with 1 req we can get max of 100 nfts and for the 10k we ve to make 100 req.
df = pd.DataFrame() # this is our data frame to collect all our nft

# lets set the params for the req we gon make to the moralis api
firstParams = {
    "address": nftContract,
    "chain": "eth",
    "disable_total": False
...
# this will give the 1st 100 result of the nft collection
result = evm_api.nft.get_contract_nfts(
    api_key=api_key,
    params=firstParams
) 
# lets get the total amt of nfts 
totalNFTs = result["total"] # the total nft is 10k
numOfReqs = math.ceil(totalNFTs/100) # the 100 is the total no of nfts that the result[] has
...
for x in range(numOfReqs):
    ...
    print(x + 1, "/", numOfReqs)
    time.sleep(0.21) # since with the free plan we can only make 5 api reqs, so this will allow us to stay with the free plan rate limit
    
# now we ve all the 10k nft collections(including the meta data) stored in our df 

# lets make all the indexes row in the df are from 1 to 10k 
...
# now that we ve our rarity scores(highes at the top and the lowest at the bottom) set for all the nft collections, convert em into the pdf
df.to_json(r'./backend/impostors.json', orient="index")
